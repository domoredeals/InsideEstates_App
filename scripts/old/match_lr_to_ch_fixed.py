#!/usr/bin/env python3
"""
Match Land Registry proprietors to Companies House data using FIXED 4-tier matching logic
This version fixes the normalization bug that was removing company suffixes
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('lr_ch_matching_fixed.log'),
...
class LandRegistryCompaniesHouseMatcherFixed:
    ...
    def normalize_company_name_fixed(self, name):
        """Fixed normalization that preserves company suffixes"""
        ...
        name = re.sub(r'\bPLC\b', 'PUBLIC LIMITED COMPANY', name)
        name = re.sub(r'\bLLP\b', 'LIMITED LIABILITY PARTNERSHIP', name)
        ...
        with tqdm(total=total_companies, desc="Loading CH data") as pbar:
            ...
            while True:
                ...
                for company_number, company_name, company_status, company_category in rows:
                    ...
                    clean_name = self.normalize_company_name_fixed(company_name)
    ...
    def print_statistics(self):
        ...
        if self.stats['total_proprietors'] > 0:
            ...
            logger.info(f"\nℹ️  With the fixed normalization, we expect the match rate to improve significantly!")
    ...
    def run(self, test_limit=None):
        ...
        try:
            ...
            logger.info("\n✅ Matching complete with FIXED normalization!")
...
def main():
    parser = argparse.ArgumentParser(description='Match Land Registry data to Companies House data with FIXED normalization')
    ...
    logger.info("\n🔧 USING FIXED NORMALIZATION")
    logger.info("This version preserves company suffixes (LIMITED, LTD, PLC) in the normalized form")
    logger.info("Expected to significantly improve match rates!\n")
