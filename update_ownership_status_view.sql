-- Update the ownership_status logic to consider change_indicator = 'D' as Historical
-- This fixes the issue where deleted records were showing as Current

-- Drop existing views
DROP VIEW IF EXISTS v_ownership_summary CASCADE;
DROP VIEW IF EXISTS v_ownership_history CASCADE;

-- Create the updated ownership history view
...
SELECT 
    ...
    -- Flag for current vs historical ownership - UPDATED LOGIC
    CASE 
...
COMMENT ON VIEW v_ownership_history IS 'Ownership history view showing only the latest record per title/owner combination. Records with change_indicator=D are marked as Historical.';
