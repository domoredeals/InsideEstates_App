-- Create a limited view that defaults to showing only recent data
-- This prevents memory issues while maintaining compatibility

-- First drop the problematic view
DROP VIEW IF EXISTS v_ownership_history CASCADE;

-- Create a view that limits results by default
...
CREATE INDEX IF NOT EXISTS idx_lr_title_postcode ON land_registry_data(title_number, postcode);

-- Add comment
COMMENT ON VIEW v_ownership_history IS 'Limited view showing only last 3 months of data by default to prevent memory issues. Use specific WHERE clauses for older data.';