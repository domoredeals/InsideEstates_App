...
-- Create a comprehensive ownership history view that properly deduplicates owners
-- Shows only the latest record per title/owner, and handles cases where the same owner appears in multiple proprietor positions
...
WITH ownership_timeline AS (
    ...
    -- Proprietor 2 (only if different from proprietor 1)
    SELECT 
        lr.id,
        lr.title_number,
        lr.file_month,
    ...
    AND lr.proprietor_2_name != ''
    AND lr.proprietor_2_name != lr.proprietor_1_name  -- Exclude duplicates
    
    UNION ALL
    
    -- Proprietor 3 (only if different from proprietors 1 and 2)
    SELECT 
        lr.id,
        lr.title_number,
        lr.file_month,
    ...
    AND lr.proprietor_3_name != ''
    AND lr.proprietor_3_name != lr.proprietor_1_name  -- Exclude duplicates
    AND lr.proprietor_3_name != lr.proprietor_2_name  -- Exclude duplicates
    
    UNION ALL
    
    -- Proprietor 4 (only if different from proprietors 1, 2, and 3)
    SELECT 
        lr.id,
        lr.title_number,
        lr.file_month,
    ...
    AND lr.proprietor_4_name != ''
    AND lr.proprietor_4_name != lr.proprietor_1_name  -- Exclude duplicates
    AND lr.proprietor_4_name != lr.proprietor_2_name  -- Exclude duplicates
    AND lr.proprietor_4_name != lr.proprietor_3_name  -- Exclude duplicates
),
-- Now rank records to get only the latest per title/owner combination
latest_ownership AS (
    SELECT *,
        ROW_NUMBER() OVER (
...
WHERE rn = 1  -- Only keep the latest record per title/owner combination
...
COMMENT ON VIEW v_ownership_history IS 'Ownership history view showing only the latest record per title/owner combination, handling duplicate owners across proprietor positions';
